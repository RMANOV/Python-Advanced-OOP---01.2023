...
santa_pos = [[i, j] for i in range(size) for j in range(size) if matrix[i][j] == "S"][0]

directions = {"up": (-1, 0), "down": (1, 0), "left": (0, -1), "right": (0, 1)}
total_nice_kids = [
    matrix[i][j] for i in range(size) for j in range(size) if matrix[i][j] == "V"
].count("V")
...
while presents > 0:
    ...
    if (
        0 <= santa_pos[0] < size
        and 0 <= santa_pos[1] < size
        and matrix[santa_pos[0]][santa_pos[1]] != "-"
    ):
        if matrix[santa_pos[0]][santa_pos[1]] == "V":
            total_nice_kids_with_presents += 1
            presents -= 1
        elif matrix[santa_pos[0]][santa_pos[1]] == "X":
            presents -= 1
...
if total_nice_kids_with_presents == total_nice_kids:
    ...
    print(
        f"No presents for {total_nice_kids - total_nice_kids_with_presents} nice kid/s."
    )


# def eat_cookie(
#     presents_left, nice_kids
# ):  # създаваме фунцкия, първи параметър останалите подаръци и втори добрите деца
#     for x, y in directions.values():  # обхождаме всяка посока от посоките
#         r = (
#             santa_pos[0] + x
#         )  # намираме реда като съберем реда на Дядо Коледа и реда от посоката
#         c = (
#             santa_pos[1] + y
#         )  # намираме колоната като съберем колоната на Дядо Коледа и тази от посоката
...
#         presents_left,
#         nice_kids,
#     )  # връщаме наличните подаръци и броя на посетените добри деца
...
# nice_kids_visited = (
#     0  # създаваме променлива, в която да пазим броя на посетените добри деца
# )
...
#         neighborhood[row][
#             santa_pos[1]
#         ] = "-"  # променяме стойността на позицията на Дядо Коледа на тире

#     total_nice_kids += line.count(
#         "V"
#     )  # добавяме броя им от реда, към общия брой добри деца

# command = input()  # прочитаме команда, опции - up, down, left, right, Christmas morning

# while (
#     command != "Christmas morning"
# ):  # развъртаме while цикъл, докато командата е различна от Christmas morning
#     santa_pos = [
#         santa_pos[0] + directions[command][0],
#         santa_pos[1] + directions[command][1],
#     ]  # обновяваме позицията на Дядо Коледа, като събираме текущата му позиция с тази от координатите

#     house = neighborhood[santa_pos[0]][
#         santa_pos[1]
#     ]  # запазваме стойността на текущата къща
...
#         presents, nice_kids_visited = eat_cookie(
#             presents, nice_kids_visited
#         )  # извикваме функцията eat_cookies()

#     neighborhood[santa_pos[0]][
#         santa_pos[1]
#     ] = "-"  # заменяме позицията, на която сме с тире
...
# if (
#     not presents and nice_kids_visited < total_nice_kids
# ):  # проверка нямаме подаръци и не сме посетили всички добри деца
#     print("Santa ran out of presents!")  # принтираме, Santa ran out of presents!

# print(
#     *[" ".join(line) for line in neighborhood], sep="\n"
# )  # принтираме матрицата, като джойнваме всеки ред по спейс

# if (
#     nice_kids_visited == total_nice_kids
# ):  # проверяваме дали всички добри деца са получили подаръци
#     print(f"Good job, Santa! {nice_kids_visited} happy nice kid/s.")  # принтираме
# else:  # ако не всички добри деца са получили подаръци
#     print(
#         f"No presents for {total_nice_kids - nice_kids_visited} nice kid/s."
#     )  # принтираме
