...
all_targets = sum([row.count("x") for row in field])
shooted_targets = []

for _ in range(count_of_commands):
    command = input().split()
    if len(shooted_targets) == all_targets:
        break
    if command[0] == "move":
        direction = command[1]
        steps = int(command[2])
        ...
        while 0 <= new_position[0] < 5 and 0 <= new_position[1] < 5:
            if field[new_position[0]][new_position[1]] == "x":
                field[new_position[0]][new_position[1]] = "."
                shooted_targets.append(f"[{new_position[0]}, {new_position[1]}]")
                break
            new_position = (
                new_position[0] + directions[direction][0],
                new_position[1] + directions[direction][1],
            )

if all_targets == len(shooted_targets):
    print(f"Training completed! All {all_targets} targets hit.")
else:
    print(
        f"Training not completed! {all_targets - len(shooted_targets)} targets left."
    )
...
# else:
#     print(
#         f"Training not completed! {count_of_commands - len(shooted_targets)} targets left."
#     )
#     print(*shooted_targets, sep=" ")
