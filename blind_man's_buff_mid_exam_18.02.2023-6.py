...
while True:
    ...
    next_pos = (player_pos[0] + directions[command][0], player_pos[1] + directions[command][1])
    if 0 <= next_pos[0] < n and 0 <= next_pos[1] < m:
        # Check if the next position is an obstacle
        if playground[next_pos[0]][next_pos[1]] == "O":
            continue
        # Check if the next position is an opponent
        elif playground[next_pos[0]][next_pos[1]] == "P":
            touched_opponents += 1
            moves_made += 1
            playground[next_pos[0]][next_pos[1]] = "-"
        # Check if the next position is empty
        elif playground[next_pos[0]][next_pos[1]] == "-":
            moves_made += 1
        player_pos = next_pos
