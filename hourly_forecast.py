...
def forecast(*args):
    ...
    locations = dict(sorted(locations.items(), key=lambda x: x[0]))
    sorted_locations = sorted(locations.items(), key=lambda x: x[1])
    # convert to dictionary - key is location, value is weather
    locations_dict = {}
    ...
    return "\n".join([f"{location} - {weather}" for location, weather in locations_dict.items()])



    # # final = [ f"{location} - {weather}" for location, weather in locations.items()]
    # # return " ".join(final)
