...
eggs = deque(map(int, input().split(", ")))  # Use deque instead of list for efficient pops
papers = deque(map(int, input().split(", ")))
eggs_counter = 0
boxes_filled = 0

while boxes_filled < 1:
    if not eggs or not papers:
        break  # Exit early if one of the sequences is exhausted
    egg = eggs.popleft()
    if egg == 13:
        papers.appendleft(papers.pop())  # Swap first and last paper if egg is 13
        continue
    if egg <= 0:
        continue
    paper = papers.pop()
    if egg + paper <= 50:
        ...
        papers.append(paper)  # Put paper back at the end of the deque
    # Note: we don't need to put the egg back because we're using a deque
...
print(f"Pieces of paper left: {', '.join(map(str, papers))}") if eggs else None
