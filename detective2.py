...
def detective():
    ...
    with open('C:/Users/r.manov/Desktop/Data.txt', 'r') as f:
        ...
        data = [{k: v for k, v in zip(['ip', 'mac', 'user', 'first', 'last', 'count', 'active'], row)}
                for row in data]
        for i in data:
    ...
    for i in range(len(data)):
        for j in range(i+1, len(data)):
            if data[i]['mac'] != data[j]['mac'] and \
                    abs((data[i]['first'] - data[j]['last']).total_seconds()) <= 420:
    ...
    paired_info = []
    for dev in data:
        paired_with = [pair for pair in paired_devices[dev['mac']] if pair['user'] != dev['user']]
        if paired_with:
    ...
    detected_devices = [info for info in paired_info if info[0] == user and any(pair['user'] == user for pair in info[6])]
