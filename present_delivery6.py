...
total_nice_kids = [matrix[i][j] for i in range(size) for j in range(size) if matrix[i][j] == "V"].count("V")
...
while presents and total_nice_kids_with_presents < total_nice_kids:
    ...
    if 0 <= r < size and 0 <= c < size:
        if matrix[r][c] == "V":
            ...
            presents, total_nice_kids_with_presents = eat_cookie(presents, total_nice_kids_with_presents)
        santa_pos = [r, c]
    if presents == 0:
        break

print("Santa ran out of presents!" if presents == 0 else "\n".join([" ".join(row) for row in matrix]))
